...
def create_table_from_generator(generator, schema):
    ...
    # Transpose data to column-oriented format
    ...
    for col_name in table1.column_names:
        ...
        for i, (v1, v2) in enumerate(zip(col1, col2)):
            if isinstance(v1, float) and isinstance(v2, float):
                if not math.isclose(
                    v1, v2, rel_tol=10 ** (-decimal), abs_tol=10 ** (-decimal)
                ):
...
class TestParquery(object):
    ...
    def helper_itt_groupby(data, keyfunc):
        groups = []
        uniquekeys = []
        data = sorted(data, key=keyfunc)  # mandatory before calling itertools groupby!
    ...
    def test_groupby_01(self):
        ...
        result_parquery = aggregate_pq(
            self.filename, groupby_cols, agg_list, as_df=False
        )
        print(result_parquery)

        # Itertools result
        print("--> Itertools")
        # Convert table to list format for itertools comparison
    ...
    def test_groupby_11(self):
        ...
        for i, (result_row, ref_row) in enumerate(zip(result_values, ref)):
            for j, (val, ref_val) in enumerate(zip(result_row, ref_row)):
                if not math.isclose(val, ref_val, rel_tol=1e-2, abs_tol=1e-2):
    ...
    def test_natural_notation(self):
        ...
        # table.column_names = ["d-1", "d-2", "m-1"]
    ...
    def test_natural_notation_2(self):
        ...
        terms_filter = [("d-1", "in", include)]
        result_parquery = aggregate_pq(
            filename, [], ["m-1"], data_filter=terms_filter, aggregate=True, as_df=False
        )
